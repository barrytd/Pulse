...
| **sysmon-execution-chain.evtx** | Full Sysmon attack chain on a finance workstation, spanning four Sysmon event types. A phishing `.docm` spawns `cmd.exe` → encoded PowerShell stager → `certutil` LOLBin download → `comsvcs.dll MiniDump` (Event 1), then `rundll32` opens a memory-read handle to LSASS (Event 10), the stager beacons to `185.220.101.34:4444` (Event 3), and exfiltrates over a long-subdomain DNS query (Event 22). Showcases the command-line, credential-access, and network visibility Sysmon adds over the Security log. | 7 | 🔴 CRITICAL · LSASS Memory Access<br/>🟠 HIGH · Suspicious Process Creation (×4)<br/>🟠 HIGH · Suspicious Network Connection<br/>🟠 HIGH · Suspicious DNS Query | **F** |
