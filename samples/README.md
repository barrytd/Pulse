...
Four synthetic `.evtx` files demonstrating different attack scenarios, plus a sample `pfirewall.log`. Upload any of them via the dashboard's drop zone (or `python main.py --logs samples/`) to see Pulse in action without needing access to a real Windows domain.
