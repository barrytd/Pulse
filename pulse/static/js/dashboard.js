...
// here because dashboard is the primary consumer; other modules pick
// them up via window.<fn> since all scripts load before any page renders.
(function () {
  'use strict';

  // ---------------------------------------------------------------
  // Shared state exposed on window
  // ---------------------------------------------------------------

  // Splunk-style dashboard filter state. Persisted in URL query params.
  window.dashFilterState = { time: 'today', sev: 'all', rule: 'all', source: 'all', q: '' };
  var dashFiltersHydrated = false;
  ...
  function downloadReport(scanId, target, e) {
    ...
    var url = window.apiExportUrl(scanId, fmt);
  ...
  function parseDashFiltersFromURL() {
    ...
    try {
      ...
      if (qp.has('time')   && validTime.indexOf(qp.get('time'))   >= 0) window.dashFilterState.time   = qp.get('time');
      if (qp.has('sev')    && validSev.indexOf(qp.get('sev'))     >= 0) window.dashFilterState.sev    = qp.get('sev');
      if (qp.has('rule'))   window.dashFilterState.rule   = qp.get('rule')   || 'all';
      if (qp.has('source')) window.dashFilterState.source = qp.get('source') || 'all';
      if (qp.has('q'))      window.dashFilterState.q      = qp.get('q')      || '';
    } catch (e) {}
  }

  // Push current filter state back to the URL bar.
  function writeDashFiltersToURL() {
    var qp = new URLSearchParams();
    var st = window.dashFilterState;
  ...
  function _dashTimeCutoff(range) {
    var now = new Date();
    if (range === 'today') {
  ...
  function filterScansByDashState(scans) {
    var cutoff = _dashTimeCutoff(window.dashFilterState.time);
    var src = window.dashFilterState.source;
  ...
  function filterFindingsByDashState(findings) {
    var sev  = window.dashFilterState.sev;
    var rule = window.dashFilterState.rule;
    var q    = (window.dashFilterState.q || '').trim().toLowerCase();
  ...
  function filterDailyByDashState(daily) {
    var cutoff = _dashTimeCutoff(window.dashFilterState.time);
  ...
  function _categoriesFromDeductions(deds) {
  ...
  function _dashFilterBarHtml(rules, sources) {
    var timeOpts = [
      { v: 'today', l: 'Today' },
      { v: '7d',    l: 'Last 7 days' },
  ...
  function dashFilterQueryKey(arg, target, e) {
  ...
  function openFindingDrawerByIdx(idx) {
    var f = _dashRecentFindings[idx];
    if (f && window.openFindingDrawer) window.openFindingDrawer(f);
  }
  ...
  async function renderDashboardPage() {
    ...
    var allScans = await window.fetchScans(200);
    var rules    = await window.fetchRuleNames();
    var dailyResp = await window.apiDailyScores(90);
    ...
    if (latestWithFindings) {
      var all = await window.fetchFindings(latestWithFindings.id);
    ...
    if (scans.length === 0) {
      var bannerMsg = filtersOn
        ...
        : (window.dashFilterState.time === 'today'
    ...
    if (window.mountDashLivePanel) window.mountDashLivePanel();
  }
  ...
  window.scoreColorClass           = scoreColorClass;
  window.formatBytes               = formatBytes;
  window.escapeHtml                = escapeHtml;
  window.attrEscape                = attrEscape;
  window._extractTime              = _extractTime;
  window._restoreSearchFocus       = _restoreSearchFocus;
  window._gradeFor                 = _gradeFor;
  window._gradeRank                = _gradeRank;
  window.showToast                 = showToast;
  window.toastError                = toastError;
  window.downloadReport            = downloadReport;
  window.sevBadge                  = sevBadge;
  window.statCard                  = statCard;
  window.buildDailyScoreTable      = buildDailyScoreTable;
  window.parseDashFiltersFromURL   = parseDashFiltersFromURL;
  window.writeDashFiltersToURL     = writeDashFiltersToURL;
  window.filterScansByDashState    = filterScansByDashState;
  window.filterFindingsByDashState = filterFindingsByDashState;
  window.filterDailyByDashState    = filterDailyByDashState;
  window._dashTimeCutoff           = _dashTimeCutoff;
  window._parseScanDate            = _parseScanDate;
  window._categoriesFromDeductions = _categoriesFromDeductions;
  window._dashFilterBarHtml        = _dashFilterBarHtml;
  window.applyDashFilters          = applyDashFilters;
  window.resetDashFilters          = resetDashFilters;
  window.dashFilterQueryKey        = dashFilterQueryKey;
  window._dashSources              = _dashSources;
  window._dashFiltersActive        = _dashFiltersActive;
  window._trendFor                 = _trendFor;
  window._renderTrend              = _renderTrend;
  window._trendStatCard            = _trendStatCard;
  window._accentForScore           = _accentForScore;
  window._mitreBarsHtml            = _mitreBarsHtml;
  window._dashFindingsHtml         = _dashFindingsHtml;
  window.openFindingDrawerByIdx    = openFindingDrawerByIdx;
  window._initScoreLineChart       = _initScoreLineChart;
