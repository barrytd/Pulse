...
(function () {
  ...
  async function renderWhitelistPage() {
    ...
    var bi = await window.apiWhitelistBuiltin();
    ...
    var rows = customRows.concat(window.whitelistState.showBuiltin ? builtinRows : []);

    var typeOpts = WL_TYPES.map(function (t) {
      var sel = window.whitelistState.addType === t.key ? ' selected' : '';
      ...
      '<div class="page-head">' +
        ...
        '<div class="page-head-actions">' +
          '<label class="form-checkbox" style="font-size:12px;">' +
            '<input type="checkbox"' + (window.whitelistState.showBuiltin ? ' checked' : '') +
    ...
    return '<table class="data-table">' +
      ...
      rows.map(function (r) {
        ...
        var action = r.origin === 'custom'
          ? '<button class="icon-btn" title="Remove" data-wl-key="' + window.escapeHtml(r.key) +
            '" data-wl-value="' + window.attrEscape(r.value) + '" data-action="removeWhitelistRowBtn">&times;</button>'
          : '';
        return '<tr>' +
          '<td>' + r.type + '</td>' +
          '<td class="mono">' + window.escapeHtml(r.value) + '</td>' +
          '<td>' + badge + '</td>' +
  ...
  function toggleBuiltinWhitelist(show, target) {
  ...
  async function addWhitelistEntry() {
    ...
    if (items.indexOf(value) !== -1) {
      window.toastError('Already in whitelist.');
      return;
    }
    items.push(value);
    ...
    body[type] = items;
    try {
      var r = await window.apiSaveWhitelist(body);
      if (!r.ok) throw new Error('Save failed');
      window.showToast('Added to whitelist');
      input.value = '';
      renderWhitelistPage();
    } catch (e) {
      window.toastError('Could not save whitelist.');
    }
  }
  ...
  async function removeWhitelistRow(key, value) {
    try {
      ...
      window.toastError('Could not remove entry.');
    }
  }

  function removeWhitelistRowBtn(arg, target) {
  ...
  window.renderWhitelistPage   = renderWhitelistPage;
  window.toggleBuiltinWhitelist = toggleBuiltinWhitelist;
  window.addWhitelistEntry      = addWhitelistEntry;
  window.removeWhitelistRow     = removeWhitelistRow;
  window.removeWhitelistRowBtn  = removeWhitelistRowBtn;
  window.setWhitelistAddType    = setWhitelistAddType;
  window.whitelistAddValueKey   = whitelistAddValueKey;
})();
