...
def init_db(db_path):
    ...
    alter_statements = (
        ...
        # Sprint 8 — report template catalog. A NULL value means the row
        # was generated by the legacy per-scan export endpoint that
        # pre-dates templates; the UI treats it as a generic 'scan
        # export' so old rows still display sensibly.
        "ALTER TABLE reports ADD COLUMN template_type TEXT",
    )
...
# --- Security PIN -----------------------------------------------------------
# Step-up credential for destructive actions. The API layer owns policy
# (requiring the password to set/change, gating which actions need a PIN);
# these helpers just persist the hash + lockout counters.

PIN_MAX_FAILED = 5          # wrong tries before lockout
PIN_LOCKOUT_MINUTES = 15    # lockout duration once tripped


def set_user_pin(db_path, user_id, pin_hash):
    """Store (or replace) a user's PIN hash and reset the lockout counters."""
    with _connect(db_path) as conn:
        conn.execute(
            "UPDATE users SET pin_hash = ?, pin_failed_count = 0, "
...
def clear_user_pin(db_path, user_id):
    """Remove a user's PIN (back to no-PIN / opt-out) + clear lockout state."""
    with _connect(db_path) as conn:
        conn.execute(
            "UPDATE users SET pin_hash = NULL, pin_failed_count = 0, "
            "pin_locked_until = NULL WHERE id = ?",
            (int(user_id),),
        )


def get_user_pin_hash(db_path, user_id):
    """Return the stored PIN hash, or None if the user has no PIN."""
    with _connect(db_path) as conn:
        row = conn.execute(
            "SELECT pin_hash FROM users WHERE id = ?", (int(user_id),)).fetchone()
    if not row:
        return None
    return row[0] if not isinstance(row, dict) else row.get("pin_hash")


def user_has_pin(db_path, user_id):
    """True if the user has a PIN configured (so sensitive actions are gated)."""
    return bool(get_user_pin_hash(db_path, user_id))


def get_pin_lockout_remaining(db_path, user_id):
    """Seconds remaining on a PIN lockout, or 0 if not locked."""
    with _connect(db_path) as conn:
        row = conn.execute(
    ...
    if not row:
        return 0
    locked_until = row[0] if not isinstance(row, dict) else row.get("pin_locked_until")
    if not locked_until:
        return 0
    try:
        until = datetime.strptime(str(locked_until)[:19], "%Y-%m-%d %H:%M:%S")
    except ValueError:
        return 0
...
def record_pin_failure(db_path, user_id):
    """Increment the failed-PIN counter; trip a lockout once it reaches the
    cap. Returns (attempts_left, lockout_seconds) — lockout_seconds > 0 means
    the account is now locked."""
    with _connect(db_path) as conn:
        row = conn.execute(
            "SELECT pin_failed_count FROM users WHERE id = ?",
            (int(user_id),)).fetchone()
        count = 0
        if row:
            count = (row[0] if not isinstance(row, dict) else row.get("pin_failed_count")) or 0
        count = int(count) + 1
        if count >= PIN_MAX_FAILED:
            until = (datetime.now() + timedelta(minutes=PIN_LOCKOUT_MINUTES))
            until_s = until.strftime("%Y-%m-%d %H:%M:%S")
            conn.execute(
                "UPDATE users SET pin_failed_count = ?, pin_locked_until = ? WHERE id = ?",
                (count, until_s, int(user_id)))
            return 0, PIN_LOCKOUT_MINUTES * 60
        conn.execute(
            "UPDATE users SET pin_failed_count = ? WHERE id = ?",
            (count, int(user_id)))
        return PIN_MAX_FAILED - count, 0


def reset_pin_failures(db_path, user_id):
    """Clear the failed-PIN counter + lockout after a correct PIN."""
    with _connect(db_path) as conn:
        conn.execute(
            "UPDATE users SET pin_failed_count = 0, pin_locked_until = NULL WHERE id = ?",
            (int(user_id),))
...
_REPORT_META_COLS = (
    ...
    " generated_at, organization_id, template_type"
)
...
def save_report(db_path, *, scan_id, fmt, filename, file_data,
    ...
    with _connect(db_path) as conn:
        ...
        cursor = conn.execute(
            ...
            "  generated_by, generated_at, organization_id, template_type)"
            " VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (
                int(scan_id) if scan_id is not None else None,
                str(fmt).lower(),
