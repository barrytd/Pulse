...
- **Actions column fixed.** Added an "Actions" header and replaced the ambiguous siren with two clear inline-SVG icons — a **document** (Generate report) and an **eye** (View findings) — each with a tooltip, **revealed on row hover** so rows aren't noisy at rest.
