...
- **Custom organisation branding** (`branding` table, `GET /api/branding`, admin-only `PUT /api/branding` + `POST/GET/DELETE /api/branding/logo`) — admins can set the organisation display name (80-char cap) and upload a company logo (PNG / JPEG / SVG, 2MB, magic-byte verified). Sidebar mounts the uploaded logo in place of the default ECG mark and swaps the "PULSE / Threat Detection" lockup for "{ORG NAME} / Powered by Pulse". Settings → Appearance gains an admin-only Organisation Branding card with inline logo preview + replace/remove controls; saves apply live to the sidebar without a page reload. Logo is stored as a BLOB so Render restarts preserve the upload
