...
## 2026-06-16 — Professional report redesign (shared system + Incident report)

The report output was an unstyled text dump. Rebuilt it as a **shared, print-friendly design system** that all templates can inherit, and redesigned the **Incident Investigation Report** on it (PDF + HTML).

- **New `pulse/reports/report_theme.py`** — one light/print theme (white page, dark-gray body, severity colors reserved for pills + accents) shared by PDF (reportlab) and HTML. Reusable components: a **brand header + "Page X of Y" footer on every page** (via a two-pass `NumberedCanvas`), **section headers** (uppercase label + colored accent bar), **severity pills**, a **classification banner** (color = highest severity in scope), **metadata grids**, **zebra-striped tables** with shaded headers, **callout boxes**, **finding cards** with a severity-colored left border, and a graceful **"None recorded"** for empty fields. Letter size, ~50px margins, monospace for hashes/refs.
- **Incident report redesigned** into 7 clean sections: title block + classification banner; an executive-summary callout with the verdict + severity count chips (e.g. a red "2 CRITICAL"); affected assets (Hosts/Accounts/Source IPs); a real timeline table (severity pills, mono timestamps, zebra); per-finding deep-dive cards (number, rule, severity pill, ref ID, MITRE pill, status badge, metadata line, description, Evidence SHA-256); remediation actions (with an empty state); and a formal chain-of-custody box (boxed report digest, manifest table with truncated-for-layout SHAs, verification callout).
- **JSON/CSV outputs unchanged**; the redesign is PDF + HTML only. 31 incident-report tests green; full suite 1196 passing.
- **Still to do:** migrate the other 8 templates (Executive Summary, Threat Detection Summary, NIST CSF, ISO 27001, Fleet Health, Board-Ready Posture, MITRE Coverage, Compliance Gap) onto the shared components so they inherit the same look — the system is built, this is the per-template wiring (see ROADMAP).

## 2026-06-16 — Fleet page redesign

- **Table cleaned up.** Constrained to ~1500px with fixed column widths and proper alignment (Host/Severity left, Scans/Findings/Last Scan right) so it reads as one cohesive block instead of justified edge-to-edge. 44px rows, hover + zebra (via `.data-table`), and a clearer score column.
- **Top filter bar** below the KPI tiles: hostname **search**, a **Risk** dropdown (All / Critical / High / Fair / Secure) and a **Status** dropdown (All / Online / Stale / Offline), with **Export CSV** moved to the right end of the bar (no longer floating above the tiles). The KPI tiles and the Risk dropdown share **one 4-band risk model** so they never disagree, and clicking a tile updates the dropdown (and vice-versa).
- **Row actions moved into the drawer.** The whole row already opens the host drawer, so the per-row hover icons were redundant and left the column looking empty at rest — the Actions column is gone and its space reclaimed by the other columns. "Generate report" and "View all findings" live in the drawer footer instead, alongside Close.
- **Sort.** Default sort is **risk worst-first** (Critical → High → Fair → Secure); column headers (Host, Latest Score, Scans, Findings, Last Scan) are clickable to re-sort, with an arrow on the active column. Plus a small **status-dot legend** (online / stale / offline).
- **Host drawer enriched.** Below the posture block it now shows a **severity-breakdown bar** (Critical/High/Medium/Low) and a **"Findings on this host"** list (top 8, most-severe-first, click to open the host's findings) with a "View all N findings" link. The generic "Next steps" paragraph is replaced by real footer actions: **Generate report**, **View all findings**, **Close** (the stale "system-scan panel" reference is gone). Fed by the new `GET /api/fleet/host/{hostname}` (severity breakdown + findings list, tenant-scoped).
