...
- **Fleet page redesign — frontend (backend done, finish next)** | The backend `GET /api/fleet/host/{hostname}` (severity breakdown + findings list) shipped 2026-06-16. The frontend rewrite is the remaining piece: **(table)** constrain to ~1500px with explicit column widths + alignment (text left, numbers right), an "Actions" header with clear report/eye icons revealed on row hover, a top filter bar (hostname search + Risk + Status dropdowns + Export CSV moved into it), risk-worst-first default sort with clickable sortable headers, 44px rows + hover/zebra polish + a status-dot legend; **(drawer)** add a severity-breakdown bar, a "Findings on this host" list (top ~8, clickable) with a "View all findings for this host" link, and real footer actions (Generate report / View all findings / Close) replacing the generic "Next steps" paragraph. Reference `.claude/skills/pulse-design.md` + `.claude/research/ui-ux-patterns.md`.
...
| 🔴 **Public-signup gate — 3 items left before opening signup to strangers** | The multi-tenant *data-isolation* core shipped (row above); these three are the remaining must-dos before flipping on **open public signup** (inviting a few trusted friends on your own key is fine without them). **1. CSRF protection** — custom-header / same-origin check on every state-changing route. Cookie auth + a browser is forgeable cross-site once the app is on the public internet; without this an attacker page could make a logged-in user's browser fire mutating requests. **2. Trusted-proxy `X-Forwarded-For` allowlist** — the rate limiter currently trusts the `X-Forwarded-For` header blindly, so behind a real proxy a client can spoof it to dodge per-IP limits (or frame another IP). Only trust XFF from a configured proxy allowlist; otherwise use the socket peer IP. **3. Org-scope the remaining admin-list surfaces** — audit `GET /api/feedback`, notifications, and any other admin list/aggregate that isn't yet org-filtered, the same way `/api/users` now is, so a hosted org admin can't see another tenant's rows. *(Until these land, run hosted mode for invited/trusted users only.)* |
...
- **Settings → Appearance: toggling light/dark mode kicks you back to the Profile tab** — changing the theme should not change the active settings tab. Likely the theme toggle triggers a full Settings re-render that resets the active tab to the default (Profile) instead of swapping only `#settings-tab-content` in place. Bites in the Settings page render / theme-toggle path ([`pulse/static/js/settings.js`](pulse/static/js/settings.js) + the `toggleDarkMode*` action in [`pulse/static/js/app.js`](pulse/static/js/app.js)). (Logged 2026-06-16)
