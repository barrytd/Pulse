...
| Priority | Item | Notes |
|---|---|---|
| 🟠 High | **Team role hierarchy — Phase 3+ (analyst queue + team organization)** | Completes the admin > manager > analyst hierarchy (schema + badges + endpoint gating already shipped in v1.8) so a small team can actually divide the work. Target structure: a **super-admin** (org owner — users, billing, system), a **security admin / manager** (assigns work, oversees the team, sets priority + due dates), and **security analysts** (work the alerts assigned to them, day by day). New: (1) **findings `priority` (P1–P4) + `due_date` + `assigned_by` columns**; (2) a **"My Queue" page** as the analyst's default landing — only their assigned, unresolved findings, sorted priority→severity→age, with overdue/due-today cues and KPI tiles (in queue / overdue / due today / resolved today); (3) a **"Team Workload" view** for managers — a card per analyst with open count, avg time-to-resolve, oldest unresolved, severity mix, clickable into that analyst's assignments; (4) an **assignment dialog** (pick analyst, set priority/due-date, add note) from the finding drawer + Findings bulk bar; (5) **sidebar role-gating** so analysts land on My Queue and don't see admin-only Settings. This is the "somewhere they can be organized" the product needs for team use + the paid Pro tier. |
