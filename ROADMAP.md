...
- 🔴 **Only Threat Detection Summary generates; all other 8 templates fail with "Unknown template '<slug>'. Phase 1 supports: threat_detection_summary"** — reproduced 2026-06-02 with screenshot from the live dashboard. Every card except Threat Detection Summary returns that toast. The error wording itself is the smoking gun: the literal string `"Phase 1 supports:"` only exists in the pre-Phase-2 dispatcher; Phases 2-5 rewrote it to a generic `"Supported: <full list>"` format. The frontend correctly POSTs `{template: "mitre_attack_coverage"}` etc.; the *server process* is still running pre-commit-`9e07c5d` code in memory because Python imports stick in `sys.modules` until the process restarts. The underlying root cause is the same as the 405 above: no auto-reload on the dev server. **Permanent fixes worth considering**: (1) ship a `make dev` / npm-script wrapper that runs uvicorn with `--reload --reload-dir pulse`, (2) add a `/api/health` field reporting commit SHA + dispatcher template count so a stale server is detectable from the UI without reading source, (3) put a one-line "Server build: {sha}" indicator in the user-menu dropdown so a glance reveals the version mismatch. Until then, the workaround is `Stop-Process` the python.exe running `main.py --api` and start fresh.
