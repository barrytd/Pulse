...
- [x] Threat intel integration — AbuseIPDB lookup module with 24h DB cache + private-IP guard, surfaces a confidence-of-abuse score / country / ISP / report count card on every public source IP in the finding-drawer; secrets handled like the SMTP password (only `api_key_set: bool` crosses the wire)
- [x] IOC lookup panel — Threat Intel page under Threat Management with paste-an-IP form, full result card, and a Recent Lookups column backed by `/api/intel/recent`. Hash support deferred (would need a separate VirusTotal client; the intel-cache schema is already keyed on (ip, source) so it's a drop-in extension)
